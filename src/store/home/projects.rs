...
fn load_stats() -> HashMap<String, RepoStats> {
    let json = include_str!("../../../assets/project-stats.json");
    serde_json::from_str(json).expect("Failed to parse project-stats.json")
}
...
pub fn get_projects() -> Vec<Project> {
    ...
    raw.into_iter()
        .map(|(slug, mut project)| {
            if let Some(s) = stats.get(slug) {
                project.stars = s.stars;
                project.forks = s.forks;
                project.last_commit = s.last_commit.clone();
            }
            project
        })
        .collect()
