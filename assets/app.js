...
function drawAtherParticle(i) {
  ...
  const noiseSteps = 6;
  const n = ather.simplex.noise3D(x * 0.0025, y * 0.0025, ather.tick * 0.0005) * TAU * noiseSteps;
  ...
  const outOfBounds = dx > ather.buffer.width || dx < 0 || dy > ather.buffer.height || dy < 0;
